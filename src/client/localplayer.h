...
public:
	...
	void move(float dtime, Environment *env, float pos_max_d);
	void move(float dtime, Environment *env, float pos_max_d,
			std::vector<CollisionInfo> *collision_info);
	// Temporary option for old move code
	void old_move(float dtime, Environment *env, float pos_max_d,
			std::vector<CollisionInfo> *collision_info);
	...
	uint8_t last_camera_fov = 0;
	uint8_t last_wanted_range = 0;
	...
	uint32_t maxHudId() const { return hud.size(); }

	uint16_t getBreath() const { return m_breath; }
	void setBreath(uint16_t breath) { m_breath = breath; }
...
private:
	void accelerate(const v3f &target_speed, const float max_increase_H,
		const float max_increase_V, const bool use_pitch);
	bool updateSneakNode(Map *map, const v3f &position, const v3f &sneak_max);
	float getSlipFactor(Environment *env, const v3f &speedH);
	void handleAutojump(float dtime, Environment *env,
	...
	float m_sneak_node_bb_ymax = 0.0f;
