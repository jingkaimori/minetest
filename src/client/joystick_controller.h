...
struct JoystickButtonCmb : public JoystickCombination {
	...
	JoystickButtonCmb(GameKeyType key, uint32_t filter_mask, uint32_t compare_mask) :
...
struct JoystickAxisCmb : public JoystickCombination {
	...
	JoystickAxisCmb(GameKeyType key, uint16_t axis_to_compare, int direction, int16_t thresh) :
...
public:
	...
	int16_t getAxisWithoutDead(JoystickAxis axis);

	float doubling_dtime;
...
private:
	...
	int16_t m_axes_vals[JA_COUNT];

	uint8_t m_joystick_id = 0;

	std::bitset<KeyType::INTERNAL_ENUM_COUNT> m_keys_down;
	std::bitset<KeyType::INTERNAL_ENUM_COUNT> m_keys_pressed;

	float m_internal_time;

	float m_past_pressed_time[KeyType::INTERNAL_ENUM_COUNT];
